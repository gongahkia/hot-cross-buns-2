...
- Temporary preview QA guard: live-account runtime writes are disabled by default only while validating read sync against real Google accounts. This is not the intended final product behavior; Hot Cross Buns 2 must support safe Google CRUD for normal personal use. Set `HCB_GOOGLE_WRITES_ENABLED=1` to wire authenticated write transports and drain queued Google mutations during explicit write QA.
